...
class Slalom3:
    ...
    def __init__(self, v, rad, rad2, n, ang1, ang2, end_pos, slip_gain, type, K, list_K_y):
        self.v = v
        self.rad = rad
        self.rad2 = rad2
        self.ang1 = ang1 * math.pi / 180
        self.ang2 = ang2 * math.pi / 180
        self.base_ang = ang1 + ang2
        self.pow_n = n
    ...
    def calc_base_time(self):
        ...
        while c < 10000000:
            t1 = t1 + tmp_dt
            if (2 * self.v / self.rad * self.Et * t1) >= self.ang1*2:
                self.base_time = t1
                self.limit_time_count = t1 * 2 / dt
                return
            c = c + 1

    def calc_base_time2(self):
        c = 0
        t1 = 0
        tmp_dt = 0.001 / 64
        ...
        while c < 10000000:
            t1 = t1 + tmp_dt
            if (2 * self.v / self.rad * self.Et * t1) >= self.ang2*2:
                self.base_time2 = t1
                self.limit_time_count2 = t1 * 2 / dt
                return
            c = c + 1
    ...
    def calcnormal(self, start_ang):
        ...
        tmp_y = 0
        state = 0
        alpha = 2 * self.v * self.v / (self.rad * self.rad * self.ang1 / 3)
        while True:
            tmp_alpha = 0
            if state == 0:
                tmp_alpha = alpha
            ...
            if state == 0:
                if tmp_theta > (self.ang1 / 3):
                    state = 1
            elif state == 1:
                if tmp_theta > (self.ang1 * 2 / 3):
                    state = 2
            elif state == 2:
                if tmp_w < 0:
                    state = 3
                if tmp_theta > (self.ang1):
                    state = 3
    ...
    def calc_offset_dist(self, start_pos_x, start_pos_y, type, offset):
        a = math.sin(self.ang1+self.ang2)
        b = math.cos(self.ang1+self.ang2)
        if (self.ang1+self.ang2) == 0:
            a = 1
            b = 0
        end_x = self.res["x"][-1]
        end_y = self.res["y"][-1]
