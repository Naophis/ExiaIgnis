...
void ControlLaw::calc(std::shared_ptr<motion_tgt_val_t>        tgt_val,
                      ...
                      bool motor_en, bool suction_en, bool search_mode,
  ...
  if (!search_mode_ && tgt_val_->motion_type == MotionType::STRAIGHT) {
    if (sensor_->axel_degenerate_x.size() > 0 &&
      ...
      tgt_val_->tgt_in.axel_degenerate_gain =
          (1 - param_->sensor_gain.front2.b) * tgt_val_->tgt_in.axel_degenerate_gain +
      ...
      if (diff == 0) diff = diff_old;
      axel_degenerate_gain = sensor_->interp1d(
          sensor_->axel_degenerate_dia_x, sensor_->axel_degenerate_dia_y, diff, false);
      if (axel_degenerate_gain < 0 && tgt_val_->tgt_in.end_v > tgt_val_->ego_in.v) {
...
float ControlLaw::check_sen_error(SensingControlType &type) {
  ...
  bool dist_check_right =
      ABS(tgt_val_->global_pos.dist - right_keep.star_dist) > prm->right_keep_dist_th;
  bool dist_check_left  =
      ABS(tgt_val_->global_pos.dist - left_keep.star_dist)  > prm->left_keep_dist_th;
  ...
  if (!search_mode_) {
    if (tgt_val_->motion_type == MotionType::WALL_OFF ||
      ...
      check_diff_right = (se->ego.right45_dist_diff < 0)
          ? ABS(se->ego.right45_dist_diff) < prm->sen_ref_p.normal.ref.kireme_r_wall_off2
          : ABS(se->ego.right45_dist_diff) < prm->sen_ref_p.normal.ref.kireme_r_wall_off;
      check_diff_left  = (se->ego.left45_dist_diff < 0)
          ? ABS(se->ego.left45_dist_diff)  < prm->sen_ref_p.normal.ref.kireme_l_wall_off2
          : ABS(se->ego.left45_dist_diff)  < prm->sen_ref_p.normal.ref.kireme_l_wall_off;
    } else {
  ...
  bool check_front_left  =
      (10 < se->ego.left90_mid_dist)  && (se->ego.left90_mid_dist  < prm->sen_ref_p.normal.exist.front);
  bool check_front_right =
      (10 < se->ego.right90_mid_dist) && (se->ego.right90_mid_dist < prm->sen_ref_p.normal.exist.front);

  if (!check_diff_right) enable_expand_right = false;
  if (!check_diff_left)  enable_expand_left  = false;
  ...
  bool range_check_right_expand =
      (1 < se->ego.right45_dist) && (se->ego.right45_dist < exist_right45_expand);
  ...
  if (!(check_front_left && check_front_right)) {
    ...
    if (!is_wall_off_mode) {
      check_left_sensor_error(error, check, range_check_left, dist_check_left,
                              check_diff_left, expand_left, range_check_left_expand);
      check_right_sensor_error(error, check, range_check_right, dist_check_right,
                               check_diff_right, expand_right, range_check_right_expand);
    } else {
  ...
  if (enable_expand_left && !enable_expand_right && check_diff_right &&
      ((1 < se->ego.right45_dist) && (se->ego.right45_dist < wall_th + 0.75f))) {
    enable_expand_right = true;
  } else if (!enable_expand_left && enable_expand_right && check_diff_left &&
             ((1 < se->ego.left45_dist) && (se->ego.left45_dist < wall_th + 0.75f))) {
    ...
    if (!(check_front_left && check_front_right)) {
      ...
      if (check == 0) {
        ...
        if (!range_check_passed_left_near && range_check_passed_right_near &&
          ...
          error += prm->sen_ref_p.normal2.ref.right45 - se->sen.r45.sensor_dist;
          check++;
        } else if (range_check_passed_left_near && !range_check_passed_right_near &&
                   !exist_right45_b) {
          error -= prm->sen_ref_p.normal2.ref.left45 - se->sen.l45.sensor_dist;
          check++;
        }
  ...
  if (check == 2) return error;
  if (check == 1) return error * 2;
  ee->sen.error_i = 0;
  ee->sen_log.gain_zz = 0;
  ee->sen_log.gain_z  = 0;
  return 0;
...
float ControlLaw::check_sen_error_dia(SensingControlType &type) {
  ...
  if (tgt_val_->tgt_in.tgt_dist > param_->sen_ctrl_front_th &&
    ...
    if (valid_right90) {
      ...
      tgt_val_->dia_state.right_old  = param_->sen_ref_p.dia.ref.right90 - se->ego.right90_mid_dist;
    ...
    if (valid_left90) {
      ...
      tgt_val_->dia_state.left_old   = param_->sen_ref_p.dia.ref.left90 - se->ego.left90_mid_dist;
...
void ControlLaw::check_fail_safe() {
  ...
  if (ABS(ee->v.error_i)   > param_->fail_check.v)        tgt_val_->fss.error = 1;
  if (ABS(ee->w.error_i)   > param_->fail_check.w)        tgt_val_->fss.error = 1;
  if (ABS(ee->ang.error_i) > param_->fail_check.ang)      tgt_val_->fss.error = 1;
  if (keep_wall_off_cnt    > param_->fail_check.wall_off)  tgt_val_->fss.error = 1;
}
...
void ControlLaw::calc_pid_val_ang() {
  ...
  set_ctrl_val(ee->ang_val,
               ...
               duty_sen,
               ee->ang.error_d,
               param_->angle_pid.p * ee->ang.error_p,
               param_->angle_pid.i * ang_error_i,
...
void ControlLaw::calc_pid_val_front_ctrl() {
  ...
  if (tgt_val_->motion_type == MotionType::FRONT_CTRL) {
    ...
    if (se->ego.front_dist < param_->cell) {
      ee->dist.error_p = se->ego.front_dist - param_->sen_ref_p.search_exist.front_ctrl;
...
void ControlLaw::check_left_sensor_error(
...
void ControlLaw::calc_front_ctrl_duty() {
  ...
  set_ctrl_val(ee->w_val,
               ...
               param_->gyro_pid.i * diff_ang,
               param_->gyro_pid.b * ee->w.error_i,
               param_->gyro_pid.d * ee->w_kf.error_d,
               ee->ang_log.gain_zz, ee->ang_log.gain_z);
  ...
  duty_front_ctrl_roll =
      param_->front_ctrl_angle_pid.p * ee->ang.error_p +
...
void ControlLaw::calc_angle_velocity_ctrl() {
  ...
  if (tgt_val_->motion_type == MotionType::NONE) {
    ...
    ee->ang_log.gain_z  = duty_roll;
    set_ctrl_val(ee->w_val,
                 ee->w.error_p, ee->w.error_i, 0, ee->w.error_d,
                 param_->gyro_pid.p * ee->w.error_p,
                 param_->gyro_pid.b * ee->w.error_i,
                 param_->gyro_pid.b * 0,
                 param_->gyro_pid.c * ee->w.error_d,
    ...
    if (param_->gyro_pid.antiwindup) {
      ...
      if (tgt_val_->motion_type == MotionType::SLALOM) {
        w_error_i = std::clamp(w_error_i * dt_,
                               -ABS(tgt_val_->tgt_in.tgt_angle),
                                ABS(tgt_val_->tgt_in.tgt_angle)) / dt_;
      } else if (tgt_val_->motion_type == MotionType::SLA_BACK_STR) {
        w_error_i = std::clamp(w_error_i * dt_,
                               -ABS(last_tgt_angle_),
                                ABS(last_tgt_angle_)) / dt_;
      }
    ...
    set_ctrl_val(ee->w_val,
                 ee->w.error_p, diff_ang, w_error_i, w_error_d,
...
void ControlLaw::summation_duty() {
  ...
  if (tgt_val_->motion_type == MotionType::FRONT_CTRL) {
    ...
    if (param_->FF_keV == 0) {
      ff_front2 = ff_roll2 = ff_duty_r2 = ff_duty_l2 = ff_friction_r = ff_friction_l = 0;
    }
...
void ControlLaw::apply_duty_limitter() {
  ...
  if (!isfinite(tgt_duty.duty_r)) tgt_duty.duty_r = 0;
  if (!isfinite(tgt_duty.duty_l)) tgt_duty.duty_l = 0;
...
void ControlLaw::limitter(float &kp, float &ki, float &kb, float &kd,
                           pid_param_t &lim) {
  if (lim.mode == 0) return;
  if (kp >  lim.p) kp =  lim.p; else if (kp < -lim.p) kp = -lim.p;
  if (ki >  lim.i) ki =  lim.i; else if (ki < -lim.i) ki = -lim.i;
  if (kb >  lim.b) kb =  lim.b; else if (kb < -lim.b) kb = -lim.b;
  if (kd >  lim.d) kd =  lim.d; else if (kd < -lim.d) kd = -lim.d;
}
...
void ControlLaw::set_next_duty(float duty_l, float duty_r, float duty_suction) {
  ...
  if (suction_en_) {
    ...
    if (duty_suction_in > 100.0f) duty_suction_in = 100.0f;

    gain_cnt += 1.0f;
    if (gain_cnt > suction_gain) gain_cnt = suction_gain;
    duty_suction_in = duty_suction_in * gain_cnt / suction_gain;
    if (duty_suction_in > 100.0f) duty_suction_in = 100.0f;
    if (!isfinite(duty_suction_in)) duty_suction_in = 0;
