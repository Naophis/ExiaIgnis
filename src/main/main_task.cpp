...
void MainTask::load_param_after() {
  ...
  planning_->ctl_.set_suction_batt_boost_table(sys_.test.suction_batt_boost_v_table,
  ...
  bool updated = false;
  {
    ...
    if (!rx_buf) {
      ...
      while (true) {
        ...
        // "am32.txt@..." アップロード + "AM32WRITE" コマンドで、物理ボタン操作
        // なしにwrite_am32_param()を実行できる(このwhileループを抜けずに
