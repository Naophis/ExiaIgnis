...
typedef struct {
  ...
  // 窓内の壁誤差そのものの上限[mm](2026-09-08)。0で無効。ang_th/err_thは
  // 「変化」しか見ないため、走り出しの壁PD過渡でヨーが極値を取る瞬間
  // (角速度0で15tick角度が動かず、横位置もまだ動いていない)に発火し、
  // 実機が−1.35°向いた状態を0と宣言していた(20260908_031634.csv idx177:
  // ang −1.35°/err −1.43mm/duty_sen −5°飽和中に発火→その後1.5mm右へ流れた)。
  // 壁PDがまだ大きく操舵している(=|err|が大きい)間は「揃った」と見なさない。
  float err_abs_th = 0.0f;
  // 壁基準ヘディングの回帰判定(2026-09-08)。slope_th>0で有効、0なら上の
  // 「窓内で変化なし」判定(ang_th/err_th)のみ。
  // 窓内(dist_mm〜2*dist_mm)の (走行距離x, 壁横位置lat−lat_k·Δang) を最小二乗
  // した勾配 h=atan(dlat/dx) が壁に対する実ヘディング。|h|<slope_th[deg] かつ
  // 残差RMS<resid_th[mm] で発火し、angを0ではなくhに置く(壁と平行になる
  // のを待つのではなく、測れた向きをそのまま使う)。
  // 従来の変化量判定は分解能が err_th/dist_mm=0.5mm/20mm≒1.4°しかなく、
  // 走り出しの壁PD過渡で−0.68°/−1.35°を0と宣言していた
  // (20260908_033618.csv idx168 / 031634.csv idx177)。回帰の勾配は巡航の
  // lat雑音σ0.1mmに対し30mm窓で0.1°程度の精度。
  float slope_th = 0.0f; // [deg] 0で無効
  float resid_th = 0.3f; // [mm]  回帰残差RMS上限(巡航0.05〜0.1、走り出し過渡0.1〜0.2)
  // 45°センサーは回転だけで見かけの横位置が動く(左旋回で左が近づき右が離れる)。
  // その感度[mm/deg、正]。窓内のジャイロ角変化Δangにこれを掛けて差し引く。
  // 実測: 走り出しの回転(並進ほぼ0)で+0.96(033618 idx5-90, r=0.97)、
  // +0.78(031634 idx84-176)。片壁時のlat(=∓err/2、error*2済み)も同程度。
  float lat_k = 0.0f;
  // 発火時に向きを実際に書き換える最小差[deg](2026-09-08)。0で常に書き換え。
  // 壁基準の推定は短い窓では±0.4°程度の雑音を持つ(パネル段差0.2〜0.3mm、
  // 壁PDのリミットサイクル±0.5°/周期120mm前後@400mm/s)。ジャイロ(hold残差
  // 込み)との差がこれ未満ならジャイロの方が信用できるので向きは触らない
  // (20260908_035048.csv idx233: ジャイロ+0.27°が実際と0.3°以内だったのに
  // 壁推定−0.49°へ書き換えて0.8°悪化)。置き方のズレ(1.5〜2°、025901/030521)
  // は差が大きいので従来通り補正される。横位置の合わせ込みは常に行う。
  float apply_th = 0.0f;
  // 2026-09-09: 発火時に向きを「フィットした進行方向h」ではなく0へリセット
  // する(1)。ユーザー要件「走り出し後、壁と並行になったら角度情報をリセット」。
  // 並行=壁PDの平衡なので機体はクラブ分(−c)右を向いているが、一回きりなので
  // 定数オフセットとして許容する。
  int zero_heading = 0;
  // 判定窓の間、壁PDの操舵量|duty_sen|[deg]がこれを超えたら窓をやり直す。
  // 0で無効。横位置の補正中(ヨーの極値付近、duty_sen飽和)での誤発火防止
  // (20260908_031634.csv idx177: duty_sen −5°飽和中に発火)。
  float duty_th = 0.0f;
  // 走行開始からこの距離[mm](global_pos.dist)を過ぎたら、並行判定を待たずに
  // 壁が見えている最初のtickで発火する。0で無効。1区画の第1直線(約94mm)では
  // 1.8m/sで壁PDの補正が終わらず、並行判定は6本中1本しか壁切れ前に成立
  // しなかった(20260909 015235/015301/012135/012336/010055/005726)。
  float force_dist = 0.0f;
  ...
  // 一度発火した後、そこからこの距離[mm]走ったら再武装し、次に壁追従が
  // 収束した時点でもう一度再アンカーする(2026-09-08)。0なら走行あたり一度。
  // 走行中にジャイロ積分のヘディングが壁と平行なのに−0.4〜−0.6°/sで流れる
  // runがあり(20260908_023014/025901.csv、静止中はバイアス無し)、一度きり
  // だと直線の途中で再びズレて旋回入口の座標系が揃わないため。
...
typedef struct {
  ...
  int lp_ms = 100;
  // 呼び出し(=ランプ開始)からの絶対時間での打ち切り[ms]。ランプ待ち中でも
  // 収束待ち中でも、ここに達したら無条件に抜ける(0以下で無効)。通常は
  // ランプ(現行テーブルで約0.65s、電圧ブースト最大でも約0.8s)+max_msの方が
  // 先に来る。ランプテーブルを遅くしたらそれに合わせて上げること。
  int total_max_ms = 1500;
} hold_settle_t;
...
typedef struct {
  ...
  // 走行開始時に限り *_keep_dist_th のヒステリシスを一度だけ外す(2026-09-08)。
  // keep_dist_th(=柱幅6mm相当の8mm)は走行中に壁が新たに見え始めてから
  // 8mm進むまで壁制御を入れない再捕捉ヒステリシスだが、停止状態からの
  // 走り出しでは star_dist≒global_pos.dist=0 のため最初の8mmが必ず
  // 無制御になっていた(20260908_031634.csv: dist=8.28mm/v=181mm/sで初めて
  // s_pid_pが立つ)。走り出しは両壁が安定して見えているので不要。
  // 1: 停止状態(NONE)からv_max>0のSTRAIGHT指令を受けた瞬間(hold()は除外)に
  //    star_distを keep_dist_th+1 だけ手前へずらし、1tick目から壁制御を
  //    有効にする。壁が一度でも範囲外になれば star_dist が更新されて通常の
  //    ヒステリシスへ自然に戻るため、効果は走行あたり一回限り。
  // 0: 従来通り。control_law.cpp ControlLaw::skip_keep_dist_once() /
  //    planning_task.cpp cp_request() 参照。
  int keep_dist_th_start_skip = 0;
