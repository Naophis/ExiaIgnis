...
// LoggingTask (Astraea 版) スタブ
// ExiaIgnis の logging_task.hpp とは別物。
class LoggingTaskMP {
public:
    void start_slalom_log() {}
    void stop_slalom_log()  {}
    void save(const std::string &)     {}
    void dump_log(const std::string &) {}
};
...
// TrajectoryCreator スタブ
class TrajectoryCreatorMP {
public:
    TurnDirection get_turn_dir(int turn_num) {
        if (turn_num == 1) return TurnDirection::Right;
        if (turn_num == 2) return TurnDirection::Left;
        return TurnDirection::None;
    }
    TurnType get_turn_type(int turn_num, bool dia = false) {
        (void)dia;
        switch (turn_num) {
        case 1: case 2: return TurnType::Normal;
        case 128:       return TurnType::None;
        case 255:       return TurnType::Finish;
        default:        return TurnType::None;
        }
    }
    Direction get_next_dir(Direction dir, TurnType type, TurnDirection turn_dir) {
        // 簡易実装: 要チューニング
        (void)type; (void)turn_dir;
        return dir;
    }
};

// WallOffController (インターフェースのみ; 実装は別ファイルで行う)
