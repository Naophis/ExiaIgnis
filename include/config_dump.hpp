...
inline void convertToJson(const wall_off_hold_dist_t& src, JsonVariant dst) {
    ...
    dst["exist_delta_dia_l"]           = src.exist_delta_dia_l;
    dst["exist_delta_dia_r"]           = src.exist_delta_dia_r;
...
inline void convertToJson(const input_param_t& src, JsonVariant dst) {
    ...
    dst["turn_duty_floor"]     = src.turn_duty_floor;
    dst["turn_duty_slew"]      = src.turn_duty_slew;
    ...
    dst["kireme_diff_noise_th"] = src.kireme_diff_noise_th;
...
inline void convertToJson(const test_mode_t& src, JsonVariant dst) {
    ...
    {
        JsonArray a = dst["suction_ramp_rate_us_x"].to<JsonArray>();
        for (float v : src.suction_ramp_rate_us_x) a.add(v);
    }
    {
        JsonArray a = dst["suction_ramp_rate_us_y"].to<JsonArray>();
        for (float v : src.suction_ramp_rate_us_y) a.add(v);
    }
    ...
    {
        JsonArray a = dst["suction_batt_gain_table"].to<JsonArray>();
        for (float v : src.suction_batt_gain_table) a.add(v);
    }
    {
