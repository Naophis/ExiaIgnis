...
| ファイル | 内容 |
|---------|------|
| `main_task.cpp` | `create()` / `run()` / パラメータロード / USB コマンド処理 |
| `main_task_run.cpp` | `run_main_mode()` / `select_run_mode()` |
| `main_task_test.cpp` | `run_test_mode()` および各テストモード実装 |
...
| `/system.json` | `system_t` | user_mode / maze_size / goals / test_mode_t |
...
stdio は USB のみ（UART 無効）。起動後のボタン待ちループ中に以下のコマンドを受け付けます:
...
| `param_set_t` | スラローム/直線パラメータセット（`map` / `map_slow` / `map_fast` / `str_map`） |
...
## TurnType / StraightType 等の enum（`include/enums.hpp`）

主要な TurnType: `None / Normal / Large / Orval / Dia45 / Dia45_2 / Dia135 / Dia135_2 / Dia90 / Finish`
